...
with col3:
    ...
    failure_type = st.selectbox('failure_type',['Wheel Defect','Bearing Failure','Track Defect','Signal Failure','Brake Failure'],0)
    maintenance_required = st.selectbox('maintenance_required',[0,1],0)
    risk_score = st.slider('risk_score',30.0,100.0,71.4)

# Taget variable - Failure Severity
test = [region, season, train_type, train_age_yrs,
       avg_speed, dist, amb_temp,
       humidity_per, rainfall, wheel_wear_per,
       track_vibration_level, rail_wear_mm, bearing_temp,
       axle_temp, brake_pad_wear_per, brake_pressure_psi,
...
if predict_button:
    if model == 'Decision Tree':
        ...
        st.success(knn.predict(test_df)[0])
